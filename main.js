...
class Sureflap extends utils.Adapter {
     ...
     *
     * @param {boolean} connected the connection status to set
     */
     ...
     * @param {boolean} isCatFlap whether it is a cat flap
     */
    setSureflapConnectToAdapter(prefix, hierarchy, hid, deviceIndex, isCatFlap) {
        ...
        if (this.objectContainsPath(this.devices[hid][deviceIndex], 'status.locking.mode')) {
            if (
                ...
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'status.locking.mode') ||
                this.devices[hid][deviceIndex].status.locking.mode !==
                    this.devicesPrev[hid][deviceIndex].status.locking.mode
            ) {
        ...
        if (this.objectContainsPath(this.devices[hid][deviceIndex], 'control.curfew')) {
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'control.curfew') ||
                JSON.stringify(this.devices[hid][deviceIndex].control.curfew) !==
                    JSON.stringify(this.devicesPrev[hid][deviceIndex].control.curfew)
            ) {
                if (
                    this.devicesPrev[hid] &&
                    this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'control.curfew') &&
                    this.isCurfewEnabled(this.devicesPrev[hid][deviceIndex].control.curfew)
                ) {
                    const objNameLastEnabledCurfew = `${prefix + hierarchy}.${this.devices[hid][deviceIndex].name}.last_enabled_curfew`;
                    this.setCurfewToAdapter(
                        objNameLastEnabledCurfew,
                        this.devicesPrev[hid][deviceIndex].control.curfew,
                    );
                }

                const objNameCurrentCurfew =
                    `${prefix + hierarchy}.${this.devices[hid][deviceIndex].name}.control` + `.current_curfew`;
                this.setCurfewToAdapter(objNameCurrentCurfew, this.devices[hid][deviceIndex].control.curfew);

                const objNameCurfewEnabled =
                    `${prefix + hierarchy}.${this.devices[hid][deviceIndex].name}.control` + `.curfew_enabled`;
                try {
                    this.setState(
                        objNameCurfewEnabled,
                        this.isCurfewEnabled(this.devices[hid][deviceIndex].control.curfew),
                        true,
                    );
                } catch (error) {
            ...
            if (this.curfewActivePrev === undefined || this.curfewActive !== this.curfewActivePrev) {
                this.setState(
                    `${prefix + hierarchy}.${this.devices[hid][deviceIndex].name}.curfew_active`,
                    this.curfewActive,
                    true,
                );
        ...
        if (isCatFlap) {
            // assigned pets type
            if (
                this.objectContainsPath(this.devices[hid][deviceIndex], 'tags') &&
                Array.isArray(this.devices[hid][deviceIndex].tags)
            ) {
                ...
                for (let t = 0; t < this.devices[hid][deviceIndex].tags.length; t++) {
                    ...
                    if (name !== undefined) {
                        ...
                        if (this.hasPetAssignedChanged(hid, deviceIndex, this.devices[hid][deviceIndex].tags[t].id)) {
                            ...
                            if (
                                !this.devicesPrev[hid] ||
                                !this.devicesPrev[hid][deviceIndex].tags[t] ||
                        ...
                        this.log.warn(
                            `could not find pet with pet tag id (${this.devices[hid][deviceIndex].tags[t].id})`,
                        );
                        this.log.debug(
                            `cat flap '${this.devices[hid][deviceIndex].name}' has ${this.devices[hid][deviceIndex].tags.length} pets assigned and household has ${this.pets.length} pets assigned.`,
                        );
                    }
                }
                ...
                for (let p = 0; p < this.pets.length; p++) {
                    if (
                        this.hasPetAssignedChanged(hid, deviceIndex, this.pets[p].tag_id) &&
                        !this.doesTagsArrayContainTagId(this.devices[hid][deviceIndex].tags, this.pets[p].tag_id)
                    ) {
                        const objName = `${prefix + hierarchy}.${this.devices[hid][deviceIndex].name}.control.pets.${
                            this.pets[p].name
                        }.type`;
    ...
    setFeederConnectToAdapter(prefix, hierarchy, hid, deviceIndex) {
        ...
        if (this.objectContainsPath(this.devices[hid][deviceIndex], 'control.lid.close_delay')) {
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'control.lid.close_delay') ||
                this.devices[hid][deviceIndex].control.lid.close_delay !==
                    this.devicesPrev[hid][deviceIndex].control.lid.close_delay
            ) {
                this.setState(
        ...
        // feeder config
        if (
            this.objectContainsPath(this.devices[hid][deviceIndex], 'control.bowls.settings') &&
            Array.isArray(this.devices[hid][deviceIndex].control.bowls.settings)
        ) {
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'control.bowls.settings') ||
                JSON.stringify(this.devices[hid][deviceIndex].control.bowls.settings) !==
                    JSON.stringify(this.devicesPrev[hid][deviceIndex].control.bowls.settings)
            ) {
                for (let b = 0; b < this.devices[hid][deviceIndex].control.bowls.settings.length; b++) {
                    this.getObject(`${objName}.bowls.${b}`, (err, obj) => {
                        if (!err && obj) {
                            if (
                                this.objectContainsPath(
                                    this.devices[hid][deviceIndex].control.bowls.settings[b],
                                    'food_type',
                                )
                            ) {
                                this.setState(
                                    `${objName}.bowls.${b}.food_type`,
                                    this.devices[hid][deviceIndex].control.bowls.settings[b].food_type,
                                    true,
                                );
                            }
                            if (
                                ...
                                this.setState(
                                    `${objName}.bowls.${b}.target`,
                                    this.devices[hid][deviceIndex].control.bowls.settings[b].target,
                                    true,
                                );
                            }
        ...
        if (
            this.objectContainsPath(this.devices[hid][deviceIndex], 'status.bowl_status') &&
            Array.isArray(this.devices[hid][deviceIndex].status.bowl_status)
        ) {
            // get feeder remaining food data from new bowl_status
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'status.bowl_status') ||
                JSON.stringify(this.devices[hid][deviceIndex].status.bowl_status) !==
                    JSON.stringify(this.devicesPrev[hid][deviceIndex].status.bowl_status)
            ) {
                this.log.silly(`Updating remaining food data from bowl_status.`);
                const bowlCount =
                    this.objectContainsPath(this.devices[hid][deviceIndex], 'control.bowls.type') &&
                    this.devices[hid][deviceIndex].control.bowls.type === FEEDER_SINGLE_BOWL
                        ? 1
                        : this.devices[hid][deviceIndex].status.bowl_status.length;
                for (let b = 0; b < bowlCount; b++) {
                    this.getObject(`${objName}.bowls.${b}`, (err, obj) => {
                        if (!err && obj) {
                            if (
                                this.objectContainsPath(
                                    this.devices[hid][deviceIndex].status.bowl_status[b],
                                    'current_weight',
                                )
                            ) {
                                this.setState(
                                    `${objName}.bowls.${b}.weight`,
                                    this.devices[hid][deviceIndex].status.bowl_status[b].current_weight,
                                    true,
                                );
                            }
                            if (
                                ...
                                this.setState(
                                    `${objName}.bowls.${b}.fill_percent`,
                                    this.devices[hid][deviceIndex].status.bowl_status[b].fill_percent,
                                    true,
                                );
                            }
                            if (
                                ...
                                this.setState(
                                    `${objName}.bowls.${b}.last_filled_at`,
                                    this.devices[hid][deviceIndex].status.bowl_status[b].last_filled_at,
                                    true,
                                );
                            }
                            if (
                                ...
                                this.setState(
                                    `${objName}.bowls.${b}.last_zeroed_at`,
                                    this.devices[hid][deviceIndex].status.bowl_status[b].last_zeroed_at,
                                    true,
                                );
                            }
            ...
            if (
                this.updateReport &&
                (this.reportPrev === undefined ||
                    this.reportPrev.length === 0 ||
                    JSON.stringify(this.report) !== JSON.stringify(this.reportPrev))
            ) {
                ...
                for (let p = 0; p < this.pets.length; p++) {
                    ...
                    if (
                        this.objectContainsPath(this.report[p], 'feeding.datapoints') &&
                        Array.isArray(this.report[p].feeding.datapoints)
                    ) {
                        for (let i = this.report[p].feeding.datapoints.length - 1; i >= 0; i--) {
                            ...
                            if (this.report[p].feeding.datapoints[i].device_id === deviceId) {
                                ...
                                if (
                                    ...
                                    lastDatapoint.to === undefined ||
                                    new Date(lastDatapoint.to) < new Date(this.report[p].feeding.datapoints[i].to)
                                ) {
                ...
                if (lastDatapoint !== undefined) {
                    ...
                    for (
                        let b = 0;
                        b <
                        (this.objectContainsPath(lastDatapoint, 'bowl_count')
                            ? lastDatapoint.bowl_count
                            : lastDatapoint.weights.length);
                        b++
                    ) {
                        this.getObject(`${objName}.bowls.${lastDatapoint.weights[b].index}`, (err, obj) => {
                            if (!err && obj) {
                                this.getState(
                                    `${objName}.bowls.${lastDatapoint.weights[b].index}.weight`,
                                    (err, obj) => {
                                        if (!err && obj) {
                                            if (obj.val !== lastDatapoint.weights[b].weight) {
                                                this.log.debug(
                                                    `updating remaining food for feeder '${this.devices[hid][deviceIndex].name}' bowl '${lastDatapoint.weights[b].index}' with '${lastDatapoint.weights[b].weight}'.`,
                                                );
                                                this.setState(
                                                    `${objName}.bowls.${lastDatapoint.weights[b].index}.weight`,
                                                    lastDatapoint.weights[b].weight,
                                                    true,
                                                );
                                            }
                                            this.warnings[FEEDER_BOWL_REMAINING_FOOD_ADAPTER_OBJECT_MISSING][
                                            ...
                                            this.log.debug(
                                                `setting remaining food for feeder '${this.devices[hid][deviceIndex].name}' bowl '${lastDatapoint.weights[b].index}' with '${lastDatapoint.weights[b].weight}'.`,
                                            );
                                            this.setState(
                                                `${objName}.bowls.${lastDatapoint.weights[b].index}.weight`,
                                                lastDatapoint.weights[b].weight,
                                            ...
                                            if (
                                                ...
                                                this.log.warn(
                                                    `got feeder remaining food data for object '${objName}.bowls.${lastDatapoint.weights[b].index}.weight' (${b}) but object does not exist. This can happen if number of bowls is changed and can be ignored.`,
                                                );
                                                this.warnings[FEEDER_BOWL_REMAINING_FOOD_ADAPTER_OBJECT_MISSING][
                                                    deviceIndex
                                                ] = true;
        ...
        if (
            ...
            if (
                ...
                this.getObject(`${objName}.water`, (err, obj) => {
                    if (!err && obj) {
                        if (
                            this.objectContainsPath(
                                this.devices[hid][deviceIndex].status.bowl_status[0],
                                'current_weight',
                            )
                        ) {
                            this.setState(
                                `${objName}.water` + `.weight`,
                                this.devices[hid][deviceIndex].status.bowl_status[0].current_weight,
                                true,
                            );
                        }
                        if (
                            ...
                            this.setState(
                                ...
                                this.devices[hid][deviceIndex].status.bowl_status[0].fill_percent,
                                true,
                            );
                        }
                        if (
                            ...
                            this.setState(
                                ...
                                this.devices[hid][deviceIndex].status.bowl_status[0].last_filled_at,
                                true,
                            );
                        }
            ...
            if (
                ...
                for (let p = 0; p < this.pets.length; p++) {
                    ...
                    if (
                        this.objectContainsPath(this.report[p], 'drinking.datapoints') &&
                        Array.isArray(this.report[p].drinking.datapoints)
                    ) {
                        for (let i = this.report[p].drinking.datapoints.length - 1; i >= 0; i--) {
                            ...
                            if (this.report[p].drinking.datapoints[i].device_id === deviceId) {
                                ...
                                if (
                                    lastDatapoint === undefined ||
                                    lastDatapoint.to === undefined ||
                                    new Date(lastDatapoint.to) < new Date(this.report[p].drinking.datapoints[i].to)
                                ) {
                ...
                if (
                    lastDatapoint !== undefined &&
                    lastDatapoint.weights !== undefined &&
                    Array.isArray(lastDatapoint.weights) &&
                    lastDatapoint.weights.length > 0
                ) {
                            ...
                            this.getState(`${objName}.water.weight`, (err, obj) => {
                                if (!err && obj) {
                                    if (obj.val !== lastDatapoint.weights[0].weight) {
                                        this.log.debug(
                                            `updating remaining water for water dispenser '${this.devices[hid][deviceIndex].name}' with '${lastDatapoint.weights[0].weight}'.`,
                                        );
     ...
     * @param {object} pet a pet object
     */
    setPetAssignedStatusToAdapter(hid, deviceIndex, objName, pet) {
        if (pet !== undefined && pet.name !== undefined && pet.tag_id !== undefined) {
            if (
                this.objectContainsPath(this.devices[hid][deviceIndex], 'tags') &&
                this.doesTagsArrayContainTagId(this.devices[hid][deviceIndex].tags, pet.tag_id)
            ) {
                if (
                    !this.devicesPrev[hid] ||
                    !this.devicesPrev[hid][deviceIndex] ||
                    !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'tags') ||
                    !this.doesTagsArrayContainTagId(this.devicesPrev[hid][deviceIndex].tags, pet.tag_id)
                ) {
    ...
    hasPetAssignedChanged(hid, deviceIndex, tagId) {
        if (
            ...
            if (
                this.doesTagsArrayContainTagId(this.devices[hid][deviceIndex].tags, tagId) !==
                this.doesTagsArrayContainTagId(this.devicesPrev[hid][deviceIndex].tags, tagId)
            ) {
                return true;
            }
        }
     ...
     *
     * @param {string} objName an object name
     * @param {object} new_curfew the new curfew to set
     */
    ...
    setBatteryStatusToAdapter(prefix, hierarchy, hid, deviceIndex) {
        if (!this.objectContainsPath(this.devices[hid][deviceIndex], 'status.battery')) {
            ...
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'status.battery') ||
                this.devices[hid][deviceIndex].status.battery !== this.devicesPrev[hid][deviceIndex].status.battery
            ) {
        ...
        if (!this.objectContainsPath(this.devices[hid][deviceIndex], 'status.battery_percentage')) {
            ...
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'status.battery_percentage') ||
                this.devices[hid][deviceIndex].status.battery_percentage !==
                    this.devicesPrev[hid][deviceIndex].status.battery_percentage
            ) {
    ...
    setSerialNumberToAdapter(prefix, hierarchy, hid, deviceIndex) {
        if (!this.devices[hid][deviceIndex].serial_number) {
            ...
            if (
                !this.devicesPrev[hid] ||
                !this.devicesPrev[hid][deviceIndex].serial_number ||
                this.devices[hid][deviceIndex].serial_number !== this.devicesPrev[hid][deviceIndex].serial_number
            ) {
    ...
    setSignalStrengthToAdapter(prefix, hierarchy, hid, deviceIndex) {
        if (!this.objectContainsPath(this.devices[hid][deviceIndex], 'status.signal.device_rssi')) {
            ...
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'status.signal.device_rssi') ||
                this.devices[hid][deviceIndex].status.signal.device_rssi !==
                    this.devicesPrev[hid][deviceIndex].status.signal.device_rssi
            ) {
        ...
        if (!this.objectContainsPath(this.devices[hid][deviceIndex], 'status.signal.hub_rssi')) {
            ...
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'status.signal.hub_rssi') ||
                this.devices[hid][deviceIndex].status.signal.hub_rssi !==
                    this.devicesPrev[hid][deviceIndex].status.signal.hub_rssi
            ) {
    ...
    setVersionsToAdapter(prefix, hid, deviceIndex) {
        ...
        if (!this.objectContainsPath(this.devices[hid][deviceIndex], 'status.version.device.hardware')) {
            ...
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'status.version.device.hardware') ||
                this.devices[hid][deviceIndex].status.version.device.hardware !==
                    this.devicesPrev[hid][deviceIndex].status.version.device.hardware
            ) {
        ...
        if (!this.objectContainsPath(this.devices[hid][deviceIndex], 'status.version.device.firmware')) {
            ...
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'status.version.device.firmware') ||
                this.devices[hid][deviceIndex].status.version.device.firmware !==
                    this.devicesPrev[hid][deviceIndex].status.version.device.firmware
            ) {
    ...
    setHubStatusToAdapter(prefix, hid, deviceIndex) {
        if (!this.objectContainsPath(this.devices[hid][deviceIndex], 'status.led_mode')) {
            ...
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'status.led_mode') ||
                this.devices[hid][deviceIndex].status.led_mode !== this.devicesPrev[hid][deviceIndex].status.led_mode
            ) {
    ...
    setOnlineStatusToAdapter(prefix, hid, deviceIndex) {
        ...
        if (!this.objectContainsPath(this.devices[hid][deviceIndex], 'status.online')) {
            ...
            if (
                !this.devicesPrev[hid] ||
                !this.objectContainsPath(this.devicesPrev[hid][deviceIndex], 'status.online') ||
                this.devices[hid][deviceIndex].status.online !== this.devicesPrev[hid][deviceIndex].status.online
            ) {
                let objName = `${prefix}.${this.devices[hid][deviceIndex].name}.` + `online`;
                if (this.hasParentDevice(this.devices[hid][deviceIndex])) {
                    objName =
                        `${prefix}.${this.getParentDeviceName(this.devices[hid][deviceIndex])}.${
                            this.devices[hid][deviceIndex].name
                        }.` + `online`;
                }
     ...
     * @param {number} petIndex a pet index
     */
    setPetNameToAdapter(prefix, name, petIndex) {
        if (
            !this.petsPrev ||
            !this.petsPrev[petIndex] ||
            !this.petsPrev[petIndex].name ||
            name !== this.petsPrev[petIndex].name
        ) {
    ...
    setPetNameAndPositionToAdapter(prefix, name, petIndex) {
        ...
        if (
            ...
            if (
                !this.petsPrev ||
                !this.petsPrev[petIndex] ||
                !this.objectContainsPath(this.petsPrev[petIndex], 'position.where') ||
                !this.objectContainsPath(this.petsPrev[petIndex], 'position.since') ||
                this.pets[petIndex].position.where !== this.petsPrev[petIndex].position.where ||
                this.pets[petIndex].position.since !== this.petsPrev[petIndex].position.since
            ) {
                const objName = `${prefix}.${name}`;
     ...
     *
     * @param {string} prefix a prefix string
     * @param {number} p a pet index
     */
    setPetFeedingToAdapter(prefix, p) {
        if (
            this.objectContainsPath(this.report[p], 'feeding.datapoints') &&
            Array.isArray(this.report[p].feeding.datapoints) &&
            this.report[p].feeding.datapoints.length > 0
        ) {
            if (
                !this.reportPrev[p] ||
                !this.reportPrev[p].feeding ||
                JSON.stringify(this.report[p].feeding) !== JSON.stringify(this.reportPrev[p].feeding)
            ) {
                const consumptionData = this.calculateFoodConsumption(p);
                this.log.debug(
                    `updating food consumed for pet '${this.pets[p].name}' with '${JSON.stringify(consumptionData)}'`,
                );
    ...
    setPetDrinkingToAdapter(prefix, p) {
        if (
            this.objectContainsPath(this.report[p], 'drinking.datapoints') &&
            Array.isArray(this.report[p].drinking.datapoints) &&
            this.report[p].drinking.datapoints.length > 0
        ) {
            if (
                !this.reportPrev[p] ||
                !this.reportPrev[p].drinking ||
                JSON.stringify(this.report[p].drinking) !== JSON.stringify(this.reportPrev[p].drinking)
            ) {
                const consumptionData = this.calculateWaterConsumption(p);
                this.log.debug(
                    `updating water consumed for pet '${this.pets[p].name}' with '${JSON.stringify(consumptionData)}'`,
                );
    ...
    setPetOutsideToAdapter(prefix, p) {
        if (
            this.objectContainsPath(this.report[p], 'movement.datapoints') &&
            Array.isArray(this.report[p].movement.datapoints) &&
            this.report[p].movement.datapoints.length > 0
        ) {
            if (
                !this.reportPrev[p] ||
                JSON.stringify(this.report[p].movement) !== JSON.stringify(this.reportPrev[p].movement)
            ) {
                const outsideData = this.calculateTimeOutside(p);
                this.log.debug(
                    `updating time outside for pet '${this.pets[p].name}' with '${JSON.stringify(outsideData)}'`,
                );
    ...
    setPetLastMovementToAdapter(prefix, petIndex, petName, hid) {
        if (
            ...
            if (
                movement !== undefined &&
                'last_direction' in movement &&
                'last_flap' in movement &&
                'last_flap_id' in movement &&
                'last_time' in movement
            ) {
     ...
     * @param {string} prefix a prefix string
     * @param {number} hid a household id
     * @param {number} index a history index
     */
    setHistoryEventToAdapter(prefix, hid, index) {
        this.createAdapterStructureFromJson(
            `${prefix}.history.${index}`,
            this.history[hid][index],
            `history event ${index}`,
        );
    }
    ...
    createAdapterStructureFromJson(prefix, json, desc) {
        if (Array.isArray(json)) {
            if (this.arrayContainsObjects(json)) {
                ...
                this.setObjectNotExists(
                    prefix,
                    this.buildStateObject(desc, desc.endsWith('_at') ? 'date' : 'indicator', 'string'),
                    () => {
            ...
            this.setObjectNotExists(
                prefix,
                this.buildStateObject(desc, desc.endsWith('_at') ? 'date' : 'indicator', typeof json),
                () => {
    ...
    resetHubLedModeToAdapter(hierarchy, hubName) {
        ...
        if (device !== undefined) {
            ...
            if (
                ...
                hid in this.devices &&
                Array.isArray(this.devices[hid]) &&
                this.objectContainsPath(this.devices[hid][hubIndex], 'status.led_mode')
            ) {
    ...
    resetFeederCloseDelayToAdapter(hierarchy, feederName) {
        ...
        if (device !== undefined) {
            ...
            if (
                ...
                hid in this.devices &&
                Array.isArray(this.devices[hid]) &&
                this.objectContainsPath(this.devices[hid][deviceIndex], 'control.lid.close_delay')
            ) {
                ...
                this.log.warn(
                    `can not reset control close delay for device '${feederName}' because there is no previous value`,
                );
            }
        } else {
    ...
    resetFlapPetTypeToAdapter(hierarchy, flapName, petName, petTag) {
        ...
        if (device !== undefined) {
            ...
            if (tagIndex !== -1 && this.objectContainsPath(this.devices[hid][deviceIndex].tags[tagIndex], 'profile')) {
                ...
                this.log.warn(
                    `can not reset pet type for device '${flapName}' and pet '${petName}' because there is no previous value`,
                );
            }
        } else {
            this.log.warn(
                `can not reset pet type for device '${flapName}' and pet '${petName}' because device was not found`,
            );
        }
    }
    ...
    resetFlapLockModeToAdapter(hierarchy, flapName) {
        ...
        if (device !== undefined) {
            ...
            if (
                ...
                hid in this.devices &&
                Array.isArray(this.devices[hid]) &&
                this.objectContainsPath(this.devices[hid][deviceIndex], 'status.locking.mode')
            ) {
                ...
                this.log.warn(
                    `can not reset control lockmode for device '${flapName}' because there is no previous value`,
                );
            }
        } else {
    ...
    resetPetAssigmentToAdapter(hierarchy, deviceName, petName) {
        ...
        if (
            ...
            petTagId !== -1 &&
            this.devices &&
            Array.isArray(this.devices[device.householdId]) &&
            this.objectContainsPath(this.devices[device.householdId][device.index], 'tags')
        ) {
            const value = this.doesTagsArrayContainTagId(this.devices[device.householdId][device.index].tags, petTagId);
            this.log.debug(
                `resetting pet assigment of ${petName} for '${deviceName}' to: ${value ? 'assigned' : 'unassigned'}`,
            );
            this.setState(`${hierarchy}.pets.${petName}.assigned`, value, true);
        } else {
            this.log.warn(
                `can not reset pet assignment of '${petName}' for '${deviceName}' because there is no previous value`,
            );
        }
    }
    ...
    resetFlapCurfewEnabledToAdapter(hierarchy, flapName) {
        ...
        if (device !== undefined) {
            ...
            if (
                ...
                hid in this.devices &&
                Array.isArray(this.devices[hid]) &&
                this.objectContainsPath(this.devices[hid][deviceIndex], 'control.curfew')
            ) {
                ...
                this.log.warn(
                    `can not reset control curfew_enabled for device '${flapName}' because there is no previous value`,
                );
            }
        } else {
    ...
    resetControlCurrentCurfewToAdapter(hierarchy, deviceName) {
        ...
        if (device !== undefined) {
            ...
            if (
                ...
                this.log.warn(
                    `can not reset control current_curfew for device '${deviceName}' because there is no previous value`,
                );
            }
        } else {
            this.log.warn(
                `can not reset control current_curfew for device '${deviceName}' because device was not found`,
            );
        }
    }
     ...
     *
     * @param {string} objName an object name
     * @returns {Promise} Promise of a curfew JSon object
     */
    getCurfewFromAdapter(objName) {
        return new Promise((resolve, reject) => {
            this.getStateValueFromAdapter(objName)
                .then(curfewJson => {
    ...
    removeDeletedAndRenamedPetsFromAdapter() {
        return new Promise(resolve => {
            ...
            for (let h = 0; h < this.households.length; h++) {
                ...
                getObjectsPromiseArray.push(
                    this.getObjectsByPatternAndType(`${this.name}.${this.instance}.${prefix}.pets.*`, 'channel', false),
                );
                    ...
                    if (this.devices[hid][d].household_id === this.households[h].id) {
                        ...
                        if (this.hasParentDevice(this.devices[hid][d])) {
                            if (
                                ...
                                ].includes(this.devices[hid][d].product_id)
                            ) {
                                const objName = `${prefix}.${this.getParentDeviceName(this.devices[hid][d])}.${
                                    this.devices[hid][d].name
                                }`;
                                getObjectsPromiseArray.push(
                                    this.getObjectsByPatternAndType(
                                        `${this.name}.${this.instance}.${objName}control.pets.*`,
                                        'channel',
                                        false,
                                    ),
                                );
            ...
            Promise.all(getObjectsPromiseArray)
                .then(objs => {
            ...
            for (let h = 0; h < this.households.length; h++) {
                ...
                for (let d = 0; d < this.devices[hid].length; d++) {
                    if (this.devices[hid][d].household_id === this.households[h].id) {
                        ...
                        if (
                            this.hasParentDevice(this.devices[hid][d]) &&
                            [DEVICE_TYPE_CAT_FLAP].includes(this.devices[hid][d].product_id)
                        ) {
                            const objName = `${prefix}.${this.getParentDeviceName(this.devices[hid][d])}.${
                                this.devices[hid][d].name
                            }`;
                            for (let p = 0; p < numPets; p++) {
                                if (
                                    !this.objectContainsPath(this.devices[hid][d], 'tags') ||
                                    !this.doesTagsArrayContainTagId(this.devices[hid][d].tags, this.pets[p].tag_id)
                                ) {
                                    getObjectsPromiseArray.push(
                                        this.getObjectsByPatternAndType(
                                            `${this.name}.${this.instance}.${objName}.control.pets.${
                    ...
                    for (let d = 0; d < this.devices[hid].length; d++) {
                        if (this.devices[hid][d].household_id === this.households[h].id) {
                            ...
                            if (!this.hasParentDevice(this.devices[hid][d])) {
                                            ...
                                            if (name !== undefined) {
                                                deletePromiseArray.push(
                                                    this.removeAssignedPetsFromPetFlap(
                                                        `${objName}.assigned_pets.${name}`,
                                                    ),
                                                );
                                            } else {
                                                this.log.warn(
                                                    `could not find pet with pet tag id (${this.devices[hid][d].tags[t].id})`,
                                                );
                                                this.log.debug(
                                                    `pet flap '${objName}' has ${this.devices[hid][d].tags.length} pets assigned and household has ${this.pets.length} pets assigned.`,
                                                );
                                            }
                                        }
                                    }
                                ...
                                if (
                                    this.devices[hid][d].product_id === DEVICE_TYPE_CAT_FLAP ||
                                    this.devices[hid][d].product_id === DEVICE_TYPE_PET_FLAP
                                ) {
    ...
    createHouseholdsAndHubsToAdapter() {
        return new Promise((resolve, reject) => {
            ...
            for (let h = 0; h < this.households.length; h++) {
                ...
                this.setObjectNotExists(
                    this.households[h].name,
                    this.buildFolderObject(`Household '${this.households[h].name_org}' (${this.households[h].id})`),
                    () => {
                        // create history folder
                        this.setObjectNotExists(
                            `${this.households[h].name}.history`,
                            this.buildFolderObject('Event History'),
                            () => {
                                this.setObjectNotExists(
                                    `${this.households[h].name}.history.json`,
                                    this.buildFolderObject('JSON'),
                                    () => {
                                        if (this.config.history_json_enable) {
                                        ...
                                        for (let d = 0; d < this.devices[hid].length; d++) {
                                            if (
                                                'product_id' in this.devices[hid][d] &&
                                                this.devices[hid][d].product_id === DEVICE_TYPE_HUB
                                            ) {
                                                const objName = `${prefix}.${this.devices[hid][d].name}`;
                                                this.setObjectNotExists(
                                                    objName,
                                                    this.buildDeviceObject(
                                                        `Hub '${this.devices[hid][d].name_org}' (${
                                                            this.devices[hid][d].id
                                                        })`,
                                                    ),
                                                    () => {
                                                        ...
                                                        promiseArray.push(
                                                            this.createVersionsToAdapter(hid, d, objName),
                                                        );
                                                        Promise.all(promiseArray)
                                                            .then(() => {
                                                                this.setObjectNotExists(
                                                                    `${objName}.control.led_mode`,
                                                                    this.buildStateObject(
                                                                        'led mode',
                                                                        'indicator',
                                                                        'number',
                                                            ...
                                                            .catch(error => {
                                                                this.log.warn(
                                                                    `could not create household and hub hierarchy (${error})`,
                                                                );
                                                                return reject();
                                                            });
                                                    },
                ...
                for (let d = 0; d < this.devices[hid].length; d++) {
                    if (this.hasParentDevice(this.devices[hid][d])) {
                        ...
                        if (
                            ...
                            this.log.debug(
                                `device without parent and with unknown id (${this.devices[hid][d].product_id}) found`,
                            );
                        }
                    }
                }
        ...
        return new Promise((resolve, reject) => {
            ...
            this.setObjectNotExists(`${objName}.signal`, this.buildChannelObject('signal strength'), () => {
                ...
                Promise.all(promiseArray)
                    .then(() => {
                        this.log.silly(
                            `adapter common status hierarchy for device ${this.devices[hid][deviceIndex].name} created`,
                        );
                        return resolve();
                    })
                    .catch(error => {
                        this.log.warn(
                            `could not create adapter common status hierarchy for device ${this.devices[hid][deviceIndex].name} (${error})`,
                        );
                        return reject();
                    });
            });
        ...
        return new Promise((resolve, reject) => {
            ...
            this.setObjectNotExists(`${objName}.version`, this.buildChannelObject('version'), () => {
                ...
                Promise.all(promiseArray)
                    .then(() => {
                        this.log.silly(
                            `adapter versions hierarchy for device ${this.devices[hid][deviceIndex].name} created`,
                        );
                        return resolve();
                    })
                    .catch(error => {
                        this.log.warn(
                            `could not create adapter versions hierarchy for device ${this.devices[hid][deviceIndex].name} (${error})`,
                        );
                        return reject();
                    });
            });
    ...
    createFlapDevicesToAdapter(hid, deviceIndex, objName, isCatFlap) {
        return new Promise((resolve, reject) => {
            ...
            this.setObjectNotExists(
                objName,
                this.buildDeviceObject(
                    `device '${this.devices[hid][deviceIndex].name_org}' (${this.devices[hid][deviceIndex].id})`,
                ),
                () => {
                    promiseArray.push(
                        this.setObjectNotExistsAsync(
                                ...
                                () => {
                                    ...
                                    if (
                                        ...
                                        Promise.all(promiseArray)
                                            ...
                                            .catch(error => {
                                                this.log.warn(
                                                    `could not create adapter feeder device hierarchy (${error})`,
                                                );
                                                return reject();
                                            });
                                    } else {
                        ...
                        this.setObjectNotExists(
                            ...
                            () => {
                                this.setObjectNotExistsAsync(
                                    ...
                                    .catch(error => {
                                        this.log.warn(
                                            `could not create adapter flap device assigned pets hierarchy (${error})`,
                                        );
                                        return reject();
                                    });
                            },
    ...
    createPetAssignedControl(hid, deviceIndex, pet, objName) {
        return new Promise((resolve, reject) => {
            if (pet !== undefined && pet.id !== undefined && pet.name !== undefined && pet.name_org !== undefined) {
                        ...
                        this.setObjectNotExists(
                            ...
                            () => {
                                this.setObjectNotExistsAsync(
                                    ...
                                    .catch(error => {
                                        this.log.warn(
                                            `could not create adapter flap device control pets hierarchy (${error})`,
                                        );
                                        return reject();
                                    });
                            },
                ...
                if (pet !== undefined) {
                    this.log.warn(
                        `provided pet has missing data: pet id (${pet.id}), pet name (${pet.name}), pet original name (${pet.name_org})`,
                    );
                    this.log.debug(
                        `device '${this.devices[hid][deviceIndex].name}' has ${this.devices[hid][deviceIndex].tags.length} pets assigned and household has ${this.pets.length} pets assigned.`,
                    );
                } else {
     ...
     *
     * @param {string} prefix a prefix string
     * @param {string} householdName a household name
     * @param {string} petName a pet name
                                                ...
                                                this.setObjectNotExists(
                                                    ...
                                                    () => {
                                                        ...
                                                        Promise.all(promiseArray)
                                                            ...
                                                            .catch(error => {
                                                                this.log.warn(
                                                                    `could not create adapter pet hierarchy (${error})`,
                                                                );
                                                                return reject();
                                                            });
                                                    },
    ...
    validateAndGetCurfewFromJsonString(jsonString, device_type) {
        try {
            ...
            if (!this.arrayContainsCurfewAttributes(jsonObject)) {
                this.log.error(
                    `could not set new curfew because: JSON array does not contain lock_time and unlock_time in format HH:MM`,
                );
                return undefined;
            }
     ...
     *
     * @param {object} curfew an array of curfew settings
     * @returns {boolean} whether the current curfew is active
     */
    isCurfewActive(curfew) {
        ...
        for (let h = 0; h < curfew.length; h++) {
            if (
                'enabled' in curfew[h] &&
                curfew[h]['enabled'] &&
                'lock_time' in curfew[h] &&
                'unlock_time' in curfew[h]
            ) {
     ...
     * @returns {boolean} true if the device has a parent device, otherwise false
     */
     ...
     *
     * @param {object} device a device object
     * @returns {string} the name of the parent device if it exists, otherwise 'undefined'
     */
     ...
     *
     * @param {Array} tags an array of tag ID
     ...
     * @returns {Array} a curfew with local times
     */
     ...
     * @returns {Array} a curfew with UTC times
     */
    ...
    getObjectValueForPath(obj, path) {
        return path
            .split('.')
            .reduce((deep_object, path_part) => (deep_object ? deep_object[path_part] : undefined), obj);
    }
    ...
    isDeviceControl(idArray) {
        return (
            idArray.length > 5 &&
            (idArray[idArray.length - 2] === 'control' ||
                (idArray[idArray.length - 4] === 'control' && idArray[idArray.length - 3] === 'pets'))
        );
    }
    ...
    isPetAssigment(idArray) {
        return (
            idArray.length > 7 &&
            idArray[idArray.length - 4] === 'control' &&
            idArray[idArray.length - 3] === 'pets' &&
            idArray[idArray.length - 1] === 'assigned'
        );
    }
     ...
     *
     * @returns {string} a ISO formatted string
     */
     ...
     *
     * @param {Date} date a date
     * @returns {string} a ISO formatted date
     */
     ...
     * @param {number} battery a battery voltage value
     * @returns {number} a battery percentage
     */
    calculateBatteryPercentage(deviceId, battery) {
        switch (deviceId) {
            case DEVICE_TYPE_FEEDER:
                ...
                return Math.round(
                    ((battery - this.config.felaqua_battery_empty) /
                        (this.config.felaqua_battery_full - this.config.felaqua_battery_empty)) *
                        100,
                );
            ...
            case DEVICE_TYPE_WATER_DISPENSER:
                ...
                return Math.round(
                    ((battery - this.config.surefeed_battery_empty) /
                        (this.config.surefeed_battery_full - this.config.surefeed_battery_empty)) *
                        100,
                );
            ...
            default:
                ...
                return Math.round(
                    ((battery - this.config.sureflap_battery_empty) /
                        (this.config.sureflap_battery_full - this.config.sureflap_battery_empty)) *
                        100,
                );
        }
    }
    ...
    checkAdapterConfig() {
        ...
        if (
            !this.config.sureflap_battery_full ||
            !this.config.sureflap_battery_empty ||
            this.config.sureflap_battery_full <= this.config.sureflap_battery_empty
        ) {
        ...
        if (
            !this.config.surefeed_battery_full ||
            !this.config.surefeed_battery_empty ||
            this.config.surefeed_battery_full <= this.config.surefeed_battery_empty
        ) {
        ...
        if (
            !this.config.felaqua_battery_full ||
            !this.config.felaqua_battery_empty ||
            this.config.felaqua_battery_full <= this.config.felaqua_battery_empty
        ) {
        ...
        if (
            this.config.history_entries === undefined ||
            typeof this.config.history_entries !== 'number' ||
            this.config.history_entries > 25 ||
            this.config.history_entries < 1
        ) {
     ...
     *
     * @param {string} name a state object name
     * @param {string} role a state object role
     * @param {string} type a state object type
     * @param {boolean} readonly whether the state is readonly
     * @param {object} states possible states
     ...
     *
     * @param {string} name a device object name
     ...
     *
     * @param {string} name a channel object name
     ...
     *
     * @param {string} name a folder object name
